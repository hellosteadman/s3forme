...
import unittest, json, httplib, s3forme

# load credentials from a predefined file
CREDENTIALS = json.load(open('../../../aws_credentials.json'))
S3FM_ACCESS_KEY_ID = CREDENTIALS['access_key']
S3FM_SECRET_ACCESS_KEY = CREDENTIALS['secret_key']
BUCKET_NAME = CREDENTIALS['bucket']


class TestS3fmAuthConnection(unittest.TestCase):

	def setUp(self):
		self.conn = s3.S3fmAuthConnection(S3FM_ACCESS_KEY_ID,
			S3FM_SECRET_ACCESS_KEY)
	...
	# here but is possible with some additional setup (set the server in @conn
	# to your vanity domain)
	...
	def run_tests(self, calling_format, location):
		...
		if location:
			# prefix the bucket name with location, or tests will fail due
			# to global S3 namespace restrictions accross regions
			bucket_name = location.lower() + '-' + BUCKET_NAME
		else:
		...
		self.assertEquals(response.http_response.status, 200,
				'put with a string argument')

		response = self.conn.put(bucket_name, key, s3.S3Object(text,
				{'title': 'title'}), {'Content-Type': 'text/plain'})

		self.assertEquals(response.http_response.status, 200,
				'put with complex argument and headers')
		...
		self.assertEquals(int(response.http_response.getheader(
				'Content-Length')), len(text), 'got content-length header')

		title_with_spaces = " \t  title with leading and trailing spaces	 "
		response = self.conn.put(bucket_name, key, s3.S3Object(text,
				{'title': title_with_spaces}), {'Content-Type': 'text/plain'})

		self.assertEquals(response.http_response.status, 200,
				'put with headers with spaces')

		response = self.conn.get(bucket_name, key)
		self.assertEquals(response.http_response.status, 200, 'get object')
		self.assertEquals(response.object.metadata,
		...
		response = self.do_delimited_list(bucket_name, False,
				{'delimiter': '/'}, 2, 1, 'root list')

		response = self.do_delimited_list(bucket_name, True,
				{'max-keys': 1, 'delimiter': '/'}, 1, 0,
				'root list with max keys of 1', 'example.txt')

		response = self.do_delimited_list(bucket_name, True,
				{'max-keys': 2, 'delimiter': '/'}, 1, 1,
				'root list with max keys of 2, page 1', 'test/')

		marker = response.next_marker

		response = self.do_delimited_list(bucket_name, False,
				{'marker': marker, 'max-keys': 2, 'delimiter': '/'}, 1, 0,
				'root list with max keys of 2, page 2')

		response = self.do_delimited_list(bucket_name, False,
				{'prefix': 'test/', 'delimiter': '/'}, 1, 0, 'test/ list')

		response = self.conn.delete(bucket_name, inner_key)
		self.assertEquals(response.http_response.status, 204,
				'delete %s' % inner_key)
		...
		self.assertEquals(response.http_response.status, 200,
				'list bucket with args')
		self.assertEquals(len(response.entries), 1,
				'got back right number of keys')
		...
		response = self.conn.list_all_my_buckets()
		self.assertEquals(response.http_response.status, 200,
				'list all my buckets again')

		self.assertEquals(len(response.entries), len(buckets) - 1,
				'bucket count is correct')

	def verify_list_bucket_response(self, response, bucket, is_truncated,
			parameters, next_marker=''):
		prefix = ''
		marker = ''
		...
		if 'max-keys' in parameters:
			self.assertEquals(parameters['max-keys'], response.max_keys,
					'max-keys should match')
		self.assertEquals(parameters['delimiter'], response.delimiter,
				'delimiter should match')
		self.assertEquals(is_truncated, response.is_truncated,
				'is_truncated should match')
		self.assertEquals(next_marker, response.next_marker,
				'next_marker should match')

	def do_delimited_list(self, bucket_name, is_truncated, parameters,
			regular_expected, common_expected, test_name, next_marker=''):
		response = self.conn.list_bucket(bucket_name, parameters)
		self.assertEquals(response.http_response.status, 200, test_name)
		self.assertEquals(regular_expected, len(response.entries),
				'right number of regular entries')
		self.assertEquals(common_expected, len(response.common_prefixes),
				'right number of common prefixes')

		self.verify_list_bucket_response(response, bucket_name, is_truncated,
				parameters, next_marker)
...
class TestQueryStringAuthGenerator(unittest.TestCase):
	...
	def setUp(self):
		self.generator = s3.QueryStringAuthGenerator(S3FM_ACCESS_KEY_ID,
				S3FM_SECRET_ACCESS_KEY)
	...
	def run_tests(self, calling_format):
		...
		self.check_url(self.generator.create_bucket(BUCKET_NAME), 'PUT', 200,
				'create_bucket')
		self.check_url(self.generator.put(BUCKET_NAME, key, ''), 'PUT', 200,
				'put object', 'test data')
		self.check_url(self.generator.get(BUCKET_NAME, key), 'GET', 200,
				'get object')
		self.check_url(self.generator.list_bucket(BUCKET_NAME), 'GET', 200,
				'list bucket')
		self.check_url(self.generator.list_all_my_buckets(), 'GET', 200,
				'list all my buckets')
		acl = self.check_url(self.generator.get_acl(BUCKET_NAME, key), 'GET',
				200, 'get acl')
		self.check_url(self.generator.put_acl(BUCKET_NAME, key, acl), 'PUT',
				200, 'put acl', acl)
		bucket_acl = self.check_url(self.generator.get_bucket_acl(BUCKET_NAME),
				'GET', 200, 'get bucket acl')
		self.check_url(self.generator.put_bucket_acl(BUCKET_NAME, bucket_acl),
				'PUT', 200, 'put bucket acl', bucket_acl)
		bucket_logging = self.check_url(self.generator.get_bucket_logging(
				BUCKET_NAME), 'GET', 200, 'get bucket logging')
		self.check_url(self.generator.put_bucket_logging(BUCKET_NAME,
				bucket_logging), 'PUT', 200, 'put bucket logging',
				bucket_logging)
		self.check_url(self.generator.delete(BUCKET_NAME, key), 'DELETE', 204,
				'delete object')
		self.check_url(self.generator.delete_bucket(BUCKET_NAME), 'DELETE',
				204, 'delete bucket')
