"""
django-storages driver for S3for.me.
Ported from the original django-storages S3 (non-boto) driver:
...
from S3forme import S3fmAuthConnection, QueryStringAuthGenerator, CallingFormat

ACCESS_KEY_NAME = getattr(settings, 'S3fm_S3_ACCESS_KEY_ID',
	getattr(settings, 'S3FM_ACCESS_KEY_ID', None)
)

SECRET_KEY_NAME = getattr(settings, 'S3fm_S3_SECRET_ACCESS_KEY',
	getattr(settings, 'S3FM_SECRET_ACCESS_KEY', None)
)

HEADERS = getattr(settings, 'S3FM_HEADERS', {})
DEFAULT_ACL = getattr(settings, 'S3FM_DEFAULT_ACL', 'public-read')
QUERYSTRING_ACTIVE = getattr(settings, 'S3FM_QUERYSTRING_ACTIVE', False)
QUERYSTRING_EXPIRE = getattr(settings, 'S3FM_QUERYSTRING_EXPIRE', 60)
SECURE_URLS = getattr(settings, 'S3FM_S3_SECURE_URLS', False)
BUCKET_PREFIX = getattr(settings, 'S3FM_BUCKET_PREFIX', '')
CALLING_FORMAT = getattr(settings, 'S3FM_CALLING_FORMAT', CallingFormat.PATH)
PRELOAD_METADATA = getattr(settings, 'S3FM_PRELOAD_METADATA', False)

IS_GZIPPED = getattr(settings, 'S3FM_IS_GZIPPED', False)
...
class S3fmStorage(Storage):
	"""S3for.me"""
	
	def __init__(self, bucket = settings.S3FM_STORAGE_BUCKET_NAME,
		...
		self.connection = S3fmAuthConnection(
	...
	def _get_connection(self):
		return S3fmAuthConnection(*self._get_access_keys())
	...
	def _put_file(self, name, content):
		...
		if response.http_response.status not in (200, 206):
			raise IOError('S3fmStorageError: %s' % response.message)
	
	def _open(self, name, mode='rb'):
		name = self._clean_name(name)
		remote_file = S3fmStorageFile(name, self, mode=mode)
		return remote_file
...
class PreloadingS3fmStorage(S3fmStorage):
	pass

class S3fmStorageFile(File):
